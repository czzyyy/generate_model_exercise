...
def res_block(x, name='res_block', reuse=False):
    with tf.variable_scope(name, reuse=reuse):
        ...
        x2 = instance_normalizer(x2, name='bn2')
        return lrelu(x2 + x)
...
def build_generator(noise, reuse=False):
    with tf.variable_scope('generator', reuse=reuse):
        ...
        h0 = instance_normalizer(h0, name='g_in0', reuse=reuse)
        h0 = lrelu(h0, name='g_l0')

        h1 = deconv2d(h0, output_size=[batch_size, s_h8, s_w8, size * 4], name='g_h1', reuse=reuse)
        h1 = instance_normalizer(h1, name='g_in1', reuse=reuse)
        h1 = lrelu(h1, name='g_l1')

        h1 = res_block(h1, name='res_block0', reuse=reuse)

        h2 = deconv2d(h1, output_size=[batch_size, s_h4, s_w4, size * 2], name='g_h2', reuse=reuse)
        h2 = instance_normalizer(h2, name='g_in2', reuse=reuse)
        h2 = lrelu(h2, name='g_l2')

        h2 = res_block(h2, name='res_block1', reuse=reuse)

        h3 = deconv2d(h2, output_size=[batch_size, s_h2, s_w2, size * 1], name='g_h3', reuse=reuse)
        h3 = instance_normalizer(h3, name='g_in3', reuse=reuse)
        h3 = lrelu(h3, name='g_l3')

        h3 = res_block(h3, name='res_block2', reuse=reuse)
...
def generate_samples(num):
    ...
    sample_imgs = build_generator(noise_imgs, reuse=tf.AUTO_REUSE)
    saver = tf.train.Saver()
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        saver.restore(sess, '/home/ziyangcheng/python_save_file/wgan/save_model/2/wgan.ckpt')
        ...
        for j in range(n_batch):
            ...
            for i in range(len(samples)):
                ...
                scipy.misc.imsave(
                    '/home/ziyangcheng/python_save_file/wgan/output/generate/2/' + str(j * batch_size + i) + 'generate.png', samples[i])
    print('generate done!')
...
def start_train(dataset, chunk_size):
    ...
    fake_imgs = build_generator(noise_imgs, reuse=False)
    ...
    with tf.name_scope('optimizer'):
        ...
        # rate = tf.train.exponential_decay(learning_rate, global_step, 1024 * 5, 0.90, staircase=True)
        # rate = tf.maximum(rate, 0.00001)
        # d_trainer = tf.train.RMSPropOptimizer(rate).minimize(d_loss, var_list=dis_vars, global_step=global_step)
        # g_trainer = tf.train.RMSPropOptimizer(rate).minimize(g_loss, var_list=gen_vars, global_step=global_step)
        ...
        # weight clipping
        clip_d = [d.assign(tf.clip_by_value(d, -0.02, 0.02)) for d in dis_vars]
    with tf.Session() as sess:
        saver = tf.train.Saver()
        # merge summary
        merged = tf.summary.merge_all()
        # choose dir
        writer = tf.summary.FileWriter('/home/ziyangcheng/python_save_file/wgan/tf_board', sess.graph)
        ...
        for e in range(training_epochs):
            for batch_i in range(chunk_size):
                ...
                noise_rand = np.random.uniform(low=-1.0, high=1.0, size=(batch_size, input_size[0], input_size[1], input_size[2]))
                if e == (training_epochs / 2):
                    noise_alpha = 1.0
                batch_data = batch_data * noise_alpha + noise_rand * (1 - noise_alpha)
                ...
                # run optimizers
                sess.run((d_trainer, clip_d), feed_dict={real_imgs: batch_data, noise_imgs: noise})
                sess.run((d_trainer, clip_d), feed_dict={real_imgs: batch_data, noise_imgs: noise2})
                # run d twice g once
                ...
                if (chunk_size * e + batch_i) % display_step == 0:
                    ...
                    # save pic
                    scipy.misc.imsave('/home/ziyangcheng/python_save_file/wgan/output/train/' +
                                      str(chunk_size * e + batch_i) + '-' + str(0) + '.png', check_imgs[0])

        print('train done')
        # save sess
        saver.save(sess, '/home/ziyangcheng/python_save_file/wgan/save_model/2/wgan.ckpt')


if __name__ == '__main__':
    data_folder = '/home/ziyangcheng/datasets/faces'
    data_imgs, chunk_s = load_img(data_folder)
    start_train(data_imgs, chunk_s)
