...
class InfoGan(object):
    ...
    def generate_samples(self, num):
        ...
        sample_imgs = self.build_generator(total_noise, train=False, reuse=tf.AUTO_REUSE)
        saver = tf.train.Saver()
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            saver.restore(sess, '/home/ziyangcheng/python_save_file/info_gan/save_model/4/info_gan.ckpt')
            ...
            for j in range(n_batch):
                ...
                for i in range(len(samples)):
                    ...
                    scipy.misc.imsave(
                        '/home/ziyangcheng/python_save_file/info_gan/output/generate/4/' + str(
    ...
    def format_generate(self):
        ...
        with tf.Session() as sess:
            ...
            f, a = plt.subplots(self.c_class_dim, 10, figsize=(10, self.c_class_dim))
            for j in range(self.c_class_dim):
                ...
                for i in range(10):
                    a[j][i].imshow(np.reshape(samples[i], (self.input_size[0], self.input_size[1], 3)))
            f.savefig('/home/ziyangcheng/python_save_file/info_gan/output/generate/4/format_one.png')

            f3, a3 = plt.subplots(self.c_class_dim, 10, figsize=(10, self.c_class_dim))
            for j in range(self.c_class_dim):
                sample_class = np.zeros(shape=(self.batch_size, self.c_class_dim)).astype(np.float32)
                sample_class[:, j] = 1.0
                ...
                for i in range(10):
                    sample_code = np.zeros(shape=(self.batch_size, self.c_code_dim)).astype(np.float32)
                    sample_code[:, 4] = -1 + 0.2 * j
                    sample_noise = np.random.uniform(-1.0, 1.0, size=(self.batch_size, self.noise_size)).astype(
                        np.float32)
                    samples = sess.run(sample_imgs,
                                       feed_dict={noise_imgs: sample_noise, c_class_input: sample_class,
                                                  c_code_input: sample_code})
                    a3[j][i].imshow(np.reshape(samples[0], (self.input_size[0], self.input_size[1], 3)))
            f3.savefig('/home/ziyangcheng/python_save_file/info_gan/output/generate/4/format_two5.png')

            print('generate done!')

    def train(self):
        files, self.chunk_size = image_load.list_out_image_files(self.data_path, self.batch_size)
        ...
        total_noise = tf.concat([noise_imgs, c_class_input, c_code_input], axis=1)
        fake_imgs = self.build_generator(total_noise)
        ...
        with tf.name_scope('optimizer'):
            ...
            c_trainer = tf.train.AdamOptimizer(self.learning_rate, beta1=0.0).minimize(c_loss, var_list=c_vars)
        with tf.Session() as sess:
            saver = tf.train.Saver()
            # merge summary
                    ...
                    if (self.chunk_size * e + batch_i) % self.display_step == 0:
                        ...
                        scipy.misc.imsave('/home/ziyangcheng/python_save_file/info_gan/output/train/' + str(
                            self.chunk_size * e + batch_i) +
                                          '-' + str(0) + '.png', check_imgs[0])

            print('train done')
            # save sess
            saver.save(sess, '/home/ziyangcheng/python_save_file/info_gan/save_model/4/info_gan.ckpt')


if __name__ == '__main__':
    # data_folder = '/home/ziyangcheng/datasets/faces'
    celebA_folder = '/home/ziyangcheng/datasets/celebA/img_align_celeba'
    infogan = InfoGan(learning_rate=0.0001, noise_size=1024, input_size=[96, 96, 3], training_epochs=30, batch_size=64,
                      display_step=512, c_class_num=2, c_code_dim=5, lamda=1, size=48, path=celebA_folder)
    infogan.train()
